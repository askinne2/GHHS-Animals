...
function ghhs_found_pets_query_number_animals()
{
    ...
    if (PLUGIN_DEBUG) echo "<p>Total Request   " . ($total_requests) . "</p>"; 
...
function ghhs_found_pets_make_request()
{
    ...
    $number_requests = ghhs_found_pets_query_number_animals();
    ...
    else
    {
        ...
        for ($i = 0; $i < $number_requests; $i++) {
            ...
            {
                if (PLUGIN_DEBUG) echo "<p>Bad wp_remote_get Request </p>"; 
            ...
            if (empty($pets))
            {
                if (PLUGIN_DEBUG) echo "<p>make_request(): No pets to json_decode </p>"; 

                return;
            } 
            //array_push($all_pets, $pets[$i]->animals);
            $all_pets[] = $pets[$i]->animals;
            if (PLUGIN_DEBUG) { echo '<pre>'; print_r($all_pets); echo '</pre>'; }
...
function ghhs_found_pets_shortcode( $attributes )
{
    ...
    foreach ($pets[$i] as $pet)
    {
        ...
        if ($pet->Type === "Cat")
        {
            ...
            if (PLUGIN_DEBUG) echo "<p></strong>added:   " . $pet->Name . ":   </strong>" . $pet->Status . "</p>";
