...
<script>
	...
	public function display_all_pictures($pets) {
		...
		echo '<!-- Slider main container -->';
		echo '<div class="swiper-container-pets swiper-slideshow">';
		echo '<!-- Additional required wrapper -->';
		echo '<div class="swiper-wrapper">';
		...
		foreach ($pets as $pet) {
			...
			echo '<a href="' . ANIMAL_LINK . $pet->ID . '" target="_blank" class="swiper-name">' . $pet->Name . '</a>';
			echo '</div>';
		}
		?>
		<!-- If we need pagination -->
...
<script>
	...
	function pet_layout($pet, $adoption_link) {
		?>
